...
JNICALL
Java_com_shopot_SharedSecretModule_sharedSecretC(JNIEnv *env, jobject obj,
                                                 jbyteArray publicKeyJava) {
    ...
    (*env)->SetByteArrayRegion(env, ciphertextJava, 0, OQS_KEM_kyber_768_length_ciphertext,
                               (jbyte *) result.ciphertext);

    jbyteArray sharedSecretJava = (*env)->NewByteArray(env, OQS_KEM_kyber_768_length_shared_secret);
    (*env)->SetByteArrayRegion(env, sharedSecretJava, 0, OQS_KEM_kyber_768_length_shared_secret,
                               (jbyte *) result.shared_secret);
